...
void*(*subroutine)(void*), void* data) {
  ...
  if (private_data) {
    for (size_t index = 0; index < count; ++index) {
      ...
      private_data[index].team_thread_amount = count;
      private_data[index].simulation = (simulation_t*) data;

      if (pthread_create(&private_data[index].thread_address,
      /*attr*/ NULL, subroutine,
      (void*) &private_data[index]) != EXIT_SUCCESS) {
...
int join_threads(size_t count, private_data_t* threads) {
  ...
  for (size_t index = 0; index < count; ++index) {
    ...
    error += pthread_join(threads[index].thread_address, /*value_ptr*/ NULL);
  }
  free(threads);
  return error;
}
