...
int create_threads(uint64_t thread_count) {
  ...
  thrd_t* threads = (thrd_t*) malloc(thread_count * sizeof(thrd_t));
  ...
  if (threads && private_data) {
    for (uint64_t thread_number = 0; thread_number < thread_count
        ; ++thread_number) {
      private_data[thread_number].thread_number = thread_number + 1;
      private_data[thread_number].thread_count = thread_count;
      /* create_thread(greet, thread_number)
       * one argument less, no attribute to share
      */
